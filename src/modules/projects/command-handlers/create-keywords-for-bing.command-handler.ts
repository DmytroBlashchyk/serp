...
@CommandHandler(CreateKeywordsForBingCommand)
export class CreateKeywordsForBingCommandHandler
  implements ICommandHandler<CreateKeywordsForBingCommand>
{
  constructor(
  ...
  /**
   * Executes the CreateKeywordsForBingCommand.
   *
   * @param {CreateKeywordsForBingCommand} command - The command containing the necessary data to create keywords for Bing.
   * @return {Promise<void>} - A promise that resolves to void once the command execution is completed.
   */
  async execute(command: CreateKeywordsForBingCommand): Promise<void> {
    this.cliLoggingService.log('Start: CreateKeywordsForBingCommandHandler');
    ...
    try {
      ...
      this.cliLoggingService.error(
        `Error: CreateKeywordsForBingCommandHandler (${JSON.stringify(
          command,
        )})`,
        error,
      );
   ...
   * @param {KeywordEntity[]} objects - An array of objects containing IDs.
   * @return {Promise<IdType[]>} A promise that resolves to an array of IDs not found in the second array of objects.
   */
